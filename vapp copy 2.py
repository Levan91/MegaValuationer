...
if 'Development' in listing_df.columns and development:
    ...
    if len(monthly_df) < 6:
        ...
        try:
            # Ensure Prophet parameters exist with defaults if not yet defined
            growth = growth if 'growth' in locals() else 'linear'
            n_changepoints = n_changepoints if 'n_changepoints' in locals() else 25
            changepoint_range = changepoint_range if 'changepoint_range' in locals() else 0.8
            changepoint_prior_scale = changepoint_prior_scale if 'changepoint_prior_scale' in locals() else 0.05
            yearly_seasonality = yearly_seasonality if 'yearly_seasonality' in locals() else True
            weekly_seasonality = weekly_seasonality if 'weekly_seasonality' in locals() else False
            daily_seasonality = daily_seasonality if 'daily_seasonality' in locals() else False
            seasonality_prior_scale = seasonality_prior_scale if 'seasonality_prior_scale' in locals() else 10.0
            interval_width = interval_width if 'interval_width' in locals() else 0.8
            cap = cap if 'cap' in locals() else None
            scenario_pct = scenario_pct if 'scenario_pct' in locals() else 0
