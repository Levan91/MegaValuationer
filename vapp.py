...
with tab4:
    ...
    # Global time period selector for all cards (for listings only)
    listings_days = st.number_input(
        "Listings: Show only listings from the last N days",
        min_value=1, max_value=365, step=1, value=120,
        key="comp_listings_days"
    )
    # Card count state
    ...
    for row_start in range(0, n_cards, 2):
        ...
        for i in range(2):
            ...
            with row_cols[i]:
                ...
                # Time period selector per card
                time_filter_mode = st.selectbox(
                    "Time Filter Mode",
                    ["Last N Days", "After Date", "From Date to Date"],
                    key=f"time_filter_mode_{card_key}"
                )
                last_n_days = None
                after_date = None
                date_range = (None, None)
                if time_filter_mode == "Last N Days":
                    last_n_days = st.number_input(
                        ...
                        min_value=1, step=1, value=365,
                        key=f"last_n_days_{card_key}"
                    )
                elif time_filter_mode == "After Date":
                    after_date = st.date_input(
                        "Select start date",
                        key=f"after_date_{card_key}"
                    )
                elif time_filter_mode == "From Date to Date":
                    date_range = st.date_input(
                        "Select date range",
                        value=(None, None),
                        key=f"date_range_{card_key}"
                    )
                ...
                if not filters_selected:
                    ...
                    continue
                # Filter transactions for this card (ignore Unit No. for main set)
                filtered_txn = all_transactions.copy()
                if development:
                    filtered_txn = filtered_txn[filtered_txn['All Developments'] == development]
                if community:
                    filtered_txn = filtered_txn[filtered_txn['Community/Building'] == community]
                if subcommunity:
                    filtered_txn = filtered_txn[filtered_txn['Sub Community / Building'] == subcommunity]
                if layout_type:
                    filtered_txn = filtered_txn[filtered_txn['Layout Type'] == layout_type]
                if bedrooms:
                    filtered_txn = filtered_txn[filtered_txn['Beds'].astype(str) == bedrooms]
                # Apply global time filter
                if not filtered_txn.empty and 'Evidence Date' in filtered_txn.columns:
                    filtered_txn['Evidence Date'] = pd.to_datetime(filtered_txn['Evidence Date'], errors='coerce')
                    if time_filter_mode == "Last N Days" and last_n_days:
                        cutoff = pd.Timestamp.now() - pd.Timedelta(days=last_n_days)
                        filtered_txn = filtered_txn[filtered_txn['Evidence Date'] >= cutoff]
                    elif time_filter_mode == "After Date" and after_date:
                        filtered_txn = filtered_txn[filtered_txn['Evidence Date'] >= pd.to_datetime(after_date)]
                    elif time_filter_mode == "From Date to Date" and date_range and all(date_range):
                        start, end = date_range
                        filtered_txn = filtered_txn[(filtered_txn['Evidence Date'] >= pd.to_datetime(start)) & (filtered_txn['Evidence Date'] <= pd.to_datetime(end))]
                filtered_txn = filtered_txn.copy()

                # Filter listings for this card (apply listings_days filter)
                ...
                if not filtered_listings.empty:
                    ...
                    if bedrooms and 'Beds' in filtered_listings.columns:
                        filtered_listings = filtered_listings[filtered_listings['Beds'].astype(str) == bedrooms]
                    # Apply listings_days filter
                    if 'Days Listed' in filtered_listings.columns:
                        filtered_listings['Days Listed'] = pd.to_numeric(filtered_listings['Days Listed'], errors='coerce')
                        filtered_listings = filtered_listings[filtered_listings['Days Listed'] <= listings_days]

                # Prepare chart
                import plotly.graph_objects as go
                fig = go.Figure()
                # Plot all matching transactions as blue dots
                if not filtered_txn.empty and 'Evidence Date' in filtered_txn.columns and 'Price (AED)' in filtered_txn.columns:
                    txn_df = filtered_txn.copy()
                    txn_df['Evidence Date'] = pd.to_datetime(txn_df['Evidence Date'], errors='coerce')
                    txn_df = txn_df.dropna(subset=['Evidence Date', 'Price (AED)'])
                    # Highlight selected unit transactions
                    if selected_unit:
                        selected_txn = txn_df[txn_df['Unit No.'] == selected_unit]
                        other_txn = txn_df[txn_df['Unit No.'] != selected_unit]
                        if not other_txn.empty:
                            fig.add_trace(go.Scatter(
                                x=other_txn['Evidence Date'],
                                y=other_txn['Price (AED)'],
                                mode='markers',
                                marker=dict(symbol='circle', size=7, opacity=0.7, color='blue'),
                                name='Other Units',
                                text=other_txn.apply(lambda row: f"Unit: {row['Unit No.']}<br>Layout: {row.get('Layout Type','')}<br>BUA: {row.get('Unit Size (sq ft)','')}<br>Plot: {row.get('Plot Size (sq ft)','')}", axis=1),
                                hovertemplate="Date: %{x|%b %d, %Y}<br>Price: AED %{y:,.0f}<br>%{text}<extra></extra>"
                            ))
                        if not selected_txn.empty:
                            fig.add_trace(go.Scatter(
                                x=selected_txn['Evidence Date'],
                                y=selected_txn['Price (AED)'],
                                mode='markers',
                                marker=dict(symbol='star', size=14, opacity=1, color='gold'),
                                name='Selected Unit',
                                text=selected_txn.apply(lambda row: f"Unit: {row['Unit No.']}<br>Layout: {row.get('Layout Type','')}<br>BUA: {row.get('Unit Size (sq ft)','')}<br>Plot: {row.get('Plot Size (sq ft)','')}", axis=1),
                                hovertemplate="Date: %{x|%b %d, %Y}<br>Price: AED %{y:,.0f}<br>%{text}<extra></extra>"
                            ))
                    else:
                        fig.add_trace(go.Scatter(
                            x=txn_df['Evidence Date'],
                            y=txn_df['Price (AED)'],
                            mode='markers',
                            marker=dict(symbol='circle', size=7, opacity=0.7, color='blue'),
                            name='Sales',
                            text=txn_df.apply(lambda row: f"Unit: {row['Unit No.']}<br>Layout: {row.get('Layout Type','')}<br>BUA: {row.get('Unit Size (sq ft)','')}<br>Plot: {row.get('Plot Size (sq ft)','')}", axis=1),
                            hovertemplate="Date: %{x|%b %d, %Y}<br>Price: AED %{y:,.0f}<br>%{text}<extra></extra>"
                        ))
                # Live listings (verified and not verified)
                if not filtered_listings.empty and 'Price (AED)' in filtered_listings.columns:
                    if 'Days Listed' in filtered_listings.columns:
                        filtered_listings['Days Listed'] = pd.to_numeric(filtered_listings['Days Listed'], errors='coerce')
                        filtered_listings['Listing Date'] = pd.to_datetime(datetime.today() - pd.to_timedelta(filtered_listings['Days Listed'], unit='D'))
                    else:
                    ...
                    if not ver_df.empty:
                        fig.add_trace(go.Scatter(
                            ...
                            y=ver_df['Price (AED)'],
                            mode='markers',
                            marker=dict(symbol='diamond', size=10, opacity=0.8, color='green'),
                            name='Verified Listings',
                            customdata=ver_df['URL'] if 'URL' in ver_df.columns else None,
                            text=ver_df.apply(lambda row: f"{int(row['Days Listed']) if pd.notnull(row.get('Days Listed')) else ''} days ago<br>{row.get('Layout Type','')}<br>{row.get('Community','')}<br>{row.get('Subcommunity','')}", axis=1) if 'Days Listed' in ver_df.columns and 'Layout Type' in ver_df.columns else "",
                            hovertemplate="Date: %{x|%b %d, %Y}<br>Price: AED %{y:,.0f}<br>%{text}<br><a href='%{customdata}' target='_blank'>View Listing</a><extra></extra>"
                        ))
                    if not nonver_df.empty:
                        fig.add_trace(go.Scatter(
                            ...
                            y=nonver_df['Price (AED)'],
                            mode='markers',
                            marker=dict(symbol='diamond', size=10, opacity=0.8, color='red'),
                            name='Non-verified Listings',
                            customdata=nonver_df['URL'] if 'URL' in nonver_df.columns else None,
                            text=nonver_df.apply(lambda row: f"{int(row['Days Listed']) if pd.notnull(row.get('Days Listed')) else ''} days ago<br>{row.get('Layout Type','')}<br>{row.get('Community','')}<br>{row.get('Subcommunity','')}", axis=1) if 'Days Listed' in nonver_df.columns and 'Layout Type' in nonver_df.columns else "",
                            hovertemplate="Date: %{x|%b %d, %Y}<br>Price: AED %{y:,.0f}<br>%{text}<br><a href='%{customdata}' target='_blank'>View Listing</a><extra></extra>"
                        ))
                fig.update_layout(
                ...
                # Metrics
                st.markdown("---")
                filtered_txn = pd.DataFrame(filtered_txn)
                n_units = int(filtered_txn['Unit No.'].nunique()) if not filtered_txn.empty and 'Unit No.' in filtered_txn.columns else 0
                st.metric("Units of this type in selection", n_units)
                n_transactions = len(filtered_txn) if not filtered_txn.empty else 0
                ...
                if 'Evidence Date' in filtered_txn.columns:
                    monthly_counts = filtered_txn.set_index('Evidence Date').resample('ME').size()
                ...
                if 'Evidence Date' in filtered_txn.columns and 'Price (AED/sq ft)' in filtered_txn.columns:
                    price_trend = filtered_txn.set_index('Evidence Date')['Price (AED/sq ft)'].resample('ME').mean()
                ...
                if 'Evidence Date' in filtered_txn.columns and 'Price (AED/sq ft)' in filtered_txn.columns:
                    last_6mo = filtered_txn[filtered_txn['Evidence Date'] >= (pd.Timestamp.now() - pd.DateOffset(months=6))]
    ...
    # Show chart and handle click events
    selected_points = plotly_events(fig, click_event=True, key=f"comp_plotly_{card_key}")
    # If a listing marker is clicked, open its URL
    import streamlit as st
    import webbrowser
    if selected_points and 'curveNumber' in selected_points[0] and 'pointIndex' in selected_points[0]:
        curve = selected_points[0]['curveNumber']
        idx = selected_points[0]['pointIndex']
        # Only handle listing markers (curve 1 or 2, assuming 0 is transactions)
        if curve in [1, 2]:
            url = None
            if 'customdata' in fig['data'][curve] and fig['data'][curve]['customdata'] is not None:
                url = fig['data'][curve]['customdata'][idx]
            if url:
...
# Ensure Beds columns are always string for all_listings and all_transactions
if 'all_listings' in locals() and not all_listings.empty and 'Beds' in all_listings.columns:
    all_listings['Beds'] = all_listings['Beds'].astype(str)
if 'all_transactions' in locals() and not all_transactions.empty and 'Beds' in all_transactions.columns:
    all_transactions['Beds'] = all_transactions['Beds'].astype(str)
