...
with st.sidebar:
    ...
    if unit_number:
        ...
        if not unit_row.empty:
            ...
            # Ensure columns are Series before calling .eq()
            dev_col = units_df["Development"] if "Development" in units_df.columns else units_df["All Developments"]
            if not isinstance(dev_col, pd.Series):
                dev_col = pd.Series(dev_col)
            com_col = units_df["Community"] if "Community" in units_df.columns else units_df["Community/Building"]
            if not isinstance(com_col, pd.Series):
                com_col = pd.Series(com_col)
            subcom_col = units_df["Subcommunity"] if "Subcommunity" in units_df.columns else units_df["Sub Community / Building"]
            if not isinstance(subcom_col, pd.Series):
                subcom_col = pd.Series(subcom_col)
            layout_type_col = units_df["Layout Type"] if "Layout Type" in units_df.columns else pd.Series([])
            if not isinstance(layout_type_col, pd.Series):
                layout_type_col = pd.Series(layout_type_col)
            mask = dev_col.eq(selected_development) & com_col.eq(selected_community) & subcom_col.isin(selected_sub_communities)
            filtered_layout_type_col = layout_type_col[mask]
            if not isinstance(filtered_layout_type_col, pd.Series):
                filtered_layout_type_col = pd.Series(filtered_layout_type_col)
            layout_type_options = sorted(filtered_layout_type_col.dropna().unique())
...
with tab3:
    ...
    with st.expander("Most Similar Past Deals", expanded=False):
        ...
        if "All Developments" in all_transactions.columns:
            categorical_features.append("All Developments")
        if "Community/Building" in all_transactions.columns:
            categorical_features.append("Community/Building")
        if "Sub Community / Building" in all_transactions.columns:
            categorical_features.append("Sub Community / Building")
        # Only allow similarity matching if unit_number is selected
        if unit_number:
            if st.button("Compute Similar Comparables", key="compute_sim"):
                with st.spinner("Computing similar past deals..."):
                    # Reduce data size: filter to relevant subset first
                    base_df = all_transactions
                    if development:
                        base_df = base_df[base_df['All Developments'] == development]
                    if community:
                        base_df = base_df[base_df['Community/Building'].isin(community)]
                    # Optionally, filter further by subcommunity if desired (omitted for now)
                    # Fit and preprocess similarity model (cached)
                    sim_df, preprocessor, knn = get_similarity_model(base_df, numeric_features, categorical_features)
                    # Find target row in sim_df
                    target_row = sim_df[sim_df["Unit No."] == unit_number]
                    if target_row.empty:
                        st.warning("Selected unit not found in filtered transactions for similarity matching.")
                    else:
                        # Preprocess target
                        ...
                        for col in similar_df[display_cols].columns:
                            if similar_df[display_cols][col].dtype == 'object':
                                similar_df[display_cols][col] = similar_df[display_cols][col].astype(str)
                        st.markdown("Top 5 most similar past transactions:")
                        st.table(similar_df[display_cols])
            else:
                st.info("Click 'Compute Similar Comparables' to run similarity matching.")
        else:
    ...
    # Use cached function to get monthly_df
    ...
    def ensure_consistent_frequency(monthly_df, freq='M'):
        """Ensure consistent frequency for Prophet"""
        monthly_df = monthly_df.set_index('ds').resample(freq).mean().reset_index()
        return monthly_df.dropna()
    
    # Data validation
    validation_issues = validate_prophet_data(monthly_df)
    if validation_issues:
        st.error("❌ Data validation failed:")
        for issue in validation_issues:
            st.error(f"• {issue}")
        st.info("💡 Try adjusting filters or time period.")
        st.stop()
    
    # Ensure consistent frequency
    monthly_df = ensure_consistent_frequency(monthly_df, freq='M')
    
    # Outlier trimming
    trim_pct = st.checkbox("Remove outliers using IQR method", value=True,
                           help="Remove extreme values using Interquartile Range method.")
    if trim_pct:
        # Use IQR method instead of simple percentiles
        ...
        before_count = len(monthly_df)
        monthly_df = monthly_df[(monthly_df['y'] >= lower_bound) & (monthly_df['y'] <= upper_bound)]
        after_count = len(monthly_df)
        
        if before_count != after_count:
            st.info(f"📊 Removed {before_count - after_count} outliers using IQR method")
    ...
    if len(monthly_df) >= 12:
        ...
        try:
            ...
            pass
        prophet_args['uncertainty_samples'] = 1000  # Ensure int type for Prophet
    ...
    if layout_type:
        listing_df = listing_df[listing_df['Layout Type'].isin(layout_type)]  # type: ignore
    ...
    # Always calculate current/default MAPE for comparison
    prophet_args = dict(
        growth=growth,
        n_changepoints=n_changepoints,
        changepoint_range=changepoint_range,
...
def compare_prophet_models(monthly_df):
    ...
    models = {
        ...
        'Linear + Holidays': {'growth': 'linear', 'holidays': events_df},
