...
@st.cache_resource
def get_similarity_model(df, numeric_features, categorical_features):
...
def load_all_transactions(transactions_dir):
    ...
    all_files = [f for f in os.listdir(transactions_dir) if f.endswith('.xlsx') and not f.startswith('~$')]
    selected_files = st.session_state.get("included_txn_files", all_files)
    files = [f for f in all_files if f in selected_files]
    dfs = []
    for file in files:
...
def _on_development_change():
    ...
    if unit_number and development:
        valid_units = filtered_transactions[
            filtered_transactions['All Developments'] == development
        ]['Unit No.'].unique()
...
def _on_community_change():
    ...
    if unit_number and community:
        valid_units = filtered_transactions[
            filtered_transactions['Community/Building'].isin(community)
        ]['Unit No.'].unique()
        if unit_number not in valid_units:
            st.session_state.pop("unit_number", None)
...
def _on_subcommunity_change():
    ...
    if unit_number and subcommunity:
        valid_units = filtered_transactions[
            filtered_transactions['Sub Community / Building'].isin(subcommunity)
        ]['Unit No.'].unique()
        if unit_number not in valid_units:
            st.session_state.pop("unit_number", None)
    else:
...
if not all_transactions.empty:
    ...
    # Ensure unit_number is valid
    unit_number = st.session_state.get("unit_number", "")
    if unit_number and unit_number not in filtered_transactions['Unit No.'].astype(str).values:
        st.session_state.pop("unit_number", None)
else:
...
with st.sidebar:
    ...
    # Allow user to select which transaction files to include
    transactions_dir = os.path.join(os.path.dirname(__file__), 'DATA', 'Transactions')
    ...
    with st.expander("📂 Select Transaction Files", expanded=False):
        st.multiselect("Include files:", options=all_txn_files, default=all_txn_files, key="included_txn_files")
    ...
    # Use all_listings columns if available, else fallback to filtered_transactions
    units_df = all_listings if 'all_listings' in locals() and not all_listings.empty else filtered_transactions
    ...
    if unit_number:
        # Filter unit_row using units_df, which now refers to filtered_transactions (not all_transactions)
    ...
    if not filtered_transactions.empty:
        ...
        unit_number_options = sorted(unit_df['Unit No.'].dropna().astype(str).unique())
    else:
        unit_number_options = []
