...
def tune_prophet_hyperparameters(monthly_df, param_grid=None, forecast_periods=6):
    """
    Simple grid search for Prophet hyperparameters.
    Returns best params and best MAPE.
    """
    ...
    for v in product(*values):
        ...
        try:
            ...
            # Use last forecast_periods for validation
            actual = monthly_df['y'][-forecast_periods:]
            pred = forecast['yhat'][-forecast_periods:]
...
if os.path.exists(rental_file_path):
    ...
    rental_df = pd.DataFrame(columns=['Unit No.', 'Contract Start', 'Contract End'])
# --- Cached loader for all transaction files ---
@cache_data
...
with tab1:
    ...
    if unit_number and 'Contract Start' in rental_df.columns:
        ...
        if not unit_rentals.empty:
            latest = unit_rentals.sort_values(by='Contract Start', ascending=False).iloc[0]
